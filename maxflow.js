...
function Maxflow(canvas,canvas2,myfont,isdigraph,nodes,edges) {
	...
	Edge.prototype.paint = function(context,edge) {	// override Edge.paint
		...
		function printFlow(context,edge) {
			...
			var rand = (Math.random() * 30) - 15
 			context.fillText(s,xc,yc+h/4 + rand);
		}
	}
	...
	function paint(canvas) {
		...
		if (isOptimal) {
			context.fillText("максимальний потiк="+flow,10,20);
		} else {
			context.fillText("потiк="+flow,10,20);
		}
		isOptimal=false;
	}

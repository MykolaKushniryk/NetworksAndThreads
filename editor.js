...
function makeEditor(){
  ...
  var textBox = body.append('textarea')
      .attr('class', 'dialog form-control my-3')
      .attr('row', 1)
      .onKey('return', textboxHandler);


  // MaxFlow
  var maxFlow = d3.select('#max_flow');

  var solveButton = d3.select('#solve')
      .attr('disabled', null)
      .attr("name", "solve")
      .attr("type", "button")
      .on('click', solveFlow);
  ...
  // var showStepsButton = d3.select('#show_steps')
  //     .attr('disabled', null)
  //     .attr("name", "showSteps")
  //     .attr("type", "button")
  ...
  function textboxHandler() {
    ...
    // Nothing selected
      ...
      alert("Виберіть вершину або ребро.");
    }

    // Changing Node Name
    if (selected_node) {
      if (contents.length > 2) {
        alert("Довжина назви <= 2");
      }
      else {
  ...
  function restart() {
    ...
    var g = path.enter().append('g')
      ...
      .on('mousedown', function(d) {
        ...
        textBox.attr('placeholder', "Введіть пропускну здатність ребра");
        restart();
      });
    ...
    gC.on('mouseover', function(d) {
      ...
      .on('mousedown', function(d) {
        ...
        textBox.attr('placeholder', "Введіть назву вибраної вершини");
  ...
  var transcribeButton = transBtnContainer.append('button')
      .attr('class', 'transcribe btn btn-outline-info mx-2 my-3 btn-sm')
      .attr("name", "solve")
      .attr("type", "button")
      .text("Зберегти")
      .on('click', function(){
